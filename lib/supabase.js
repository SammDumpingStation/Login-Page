...
export const updateUser = async (userId, data) => {
  try {
    const updatedData = {
      name: data.fullName,
      phone_number: data.phoneNumber, // Set to null if empty
    };
    ...
    const { data: updatedUser, error: userError } = await supabase
      ...
      .eq("id", userId)
      .select("*")
      .single(); // Or select specific fields you want returned
...
export const logOut = async () => {
  ...
  // Return error only if there's an actual error from Supabase
  return { error };
};
